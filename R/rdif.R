...
rdif.default <- function(x, data, score=NULL, group, focal.name, D=1, alpha=0.05, missing=NA, purify=FALSE,
  ...
  if(purify) {
    ...
    dif_item <- NULL
    dif_stat <-
      data.frame(id=rep(NA_character_, nrow(x)), rdifr=NA, z.rdifr=NA,
    ...
    mmt_df <-
      data.frame(id=rep(NA_character_, nrow(x)), mu.rdifr=NA, sigma.rdifr=NA,
...
rdif_one <- function(x, data, score, group, focal.name, D=1, alpha=0.05) {
  ...
  stat_df <-
    data.frame(id=x$id, rdifr=round(rdifr, 4), z.rdifr=round(z_stat_rdifr, 4),
