...
confirm_df <- function(x, g2na = FALSE) {
  ...
  catsGood <- all(x[,2] >= 1)
  if(!modelGood) stop(paste0("At least, one model name is mis-specified under the model column. \n",
                             "Available model names are 1PLM, 2PLM, 3PLM, DRM, GRM, and GPCM"), call.=FALSE)
  if(!catsGood) stop(paste0("At least, one score category is less than 2 under the cats column.\n",
                            "Any score category should have the number greater than 1"), call.=FALSE)
