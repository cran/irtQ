...
lwrc <- function(x=NULL, theta, prob=NULL, cats, D=1) {
  ...
  if(is.null(x)) {
    ...
    if(n.prob != n.cats) {
      stop(paste0("There are ", n.prob, " items in the probability matrix (or data.frame), ",
                  "whereas there are ", n.cats, " items in the category vector."), call.=FALSE)
    }
    ...
    p <- lwRecurive(prob.cats=prepdat$prob.cat,
                    cats=prepdat$cats,
                    n.theta=n.theta)
