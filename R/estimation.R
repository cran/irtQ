...
estimation1 <- function(f_i, r_i, s_i, theta, mod=c("1PLM", "2PLM", "3PLM", "GRM", "GPCM"), score.cat, D=1, nstd,
  ...
  if(!fix.a.1pl & mod == "1PLM") {
    ...
    est <-
      suppressWarnings(
        tryCatch({stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=theta, mod=mod, D=D,
    ...
    if(is.null(est) || est$convergence > 0L) {
      ...
      est <- stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=theta, mod=mod, D=D,
                           nstd=nstd, fix.a=fix.a.1pl, fix.g=fix.g, a.val=a.val.1pl, g.val=g.val, n.1PLM=n.1PLM,
                           aprior=aprior, bprior=bprior, gprior=gprior,
                           use.aprior=use.aprior, use.bprior=use.bprior, use.gprior=use.gprior,
                           gradient=grad_item_drm,
    ...
    hess <- hess_item_drm(est$par, f_i=f_i, r_i=r_i, s_i=s_i, theta=theta, mod=mod, D=D, nstd=nstd,
    ...
    if(mod %in% c("1PLM", "2PLM", "3PLM")) {
      ...
      tmp_est[[1]] <- stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=theta, mod=mod, D=D,
      ...
      tmp_est[[2]] <- stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=theta, mod=mod, D=D,
                                    nstd=nstd, fix.a=fix.a.1pl, fix.g=fix.g, a.val=a.val.1pl, g.val=g.val, n.1PLM=NULL,
                                    aprior=aprior, bprior=bprior, gprior=gprior,
                                    use.aprior=use.aprior, use.bprior=use.bprior, use.gprior=use.gprior,
                                    gradient=grad_item_drm,
                                    control=list(eval.max=100, iter.max=50, trace=0, step.min=1, step.max=2), lower=lower, upper=upper)
      tmp_est[[3]] <- stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=theta, mod=mod, D=D,
                                    nstd=nstd, fix.a=fix.a.1pl, fix.g=fix.g, a.val=a.val.1pl, g.val=g.val, n.1PLM=NULL,
                                    aprior=aprior, bprior=bprior, gprior=gprior,
                                    use.aprior=use.aprior, use.bprior=use.bprior, use.gprior=use.gprior,
                                    gradient=grad_item_drm,
      ...
      # estimate the standard error of estimates
      hess <- hess_item_drm(est$par, f_i=f_i, r_i=r_i, s_i=s_i, theta=theta, mod=mod, D=D,
                            nstd=nstd, fix.a=fix.a.1pl, fix.g=fix.g, a.val=a.val.1pl, g.val=g.val, n.1PLM=NULL,
                            aprior=aprior, bprior=bprior, gprior=gprior,
                            use.aprior=use.aprior, use.bprior=use.bprior, use.gprior=use.gprior,
                            adjust=FALSE)
      ...
      # initial estimation to find better starting values
      tmp_est <- stats::nlminb(startval, objective=loglike_prm, r_i=r_i, theta=theta, pr.mod=mod, D=D,
      ...
      est <-
        suppressWarnings(
          tryCatch({stats::nlminb(startval, objective=loglike_prm, r_i=r_i, theta=theta, pr.mod=mod, D=D,
                                  nstd=nstd, fix.a=fix.a.gpcm, a.val=a.val.gpcm,
                                  aprior=aprior, bprior=bprior, use.aprior=use.aprior, use.bprior=use.bprior,
                                  gradient=grad_item_prm,
      ...
      if(is.null(est) || est$convergence > 0L) {
        # if error or non-convergence, only use the gradient
        est <- stats::nlminb(startval, objective=loglike_prm, r_i=r_i, theta=theta, pr.mod=mod, D=D,
                             nstd=nstd, fix.a=fix.a.gpcm, a.val=a.val.gpcm,
                             aprior=aprior, bprior=bprior, use.aprior=use.aprior, use.bprior=use.bprior,
                             gradient=grad_item_prm,
                             # hessian=hess_item_prm,
      ...
      # estimate the standard error of estimates
      hess <- hess_item_prm(est$par, r_i=r_i, theta=theta, pr.mod=mod, D=D,
                            nstd=nstd, fix.a=fix.a.gpcm, a.val=a.val.gpcm,
                            aprior=aprior, bprior=bprior, use.aprior=use.aprior, use.bprior=use.bprior,
                            adjust=FALSE)
...
estimation2 <- function(f_i, r_i, s_i, quadpt, mod=c("1PLM", "2PLM", "3PLM", "GRM", "GPCM"), score.cat, D=1, n.quad,
  ...
  if(!fix.a.1pl & mod == "1PLM") {
    ...
    est <-
      suppressWarnings(
        tryCatch({stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=quadpt,
    ...
    if(is.null(est) || est$convergence > 0L) {
      # if error or non-convergence, only use the gradient
      est <- stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=quadpt,
                           mod=mod, D=D, nstd=n.quad, fix.a=fix.a.1pl, fix.g=fix.g, a.val=a.val.1pl,
                           g.val=g.val, n.1PLM=n.1PLM, aprior=aprior, bprior=bprior, gprior=gprior,
                           use.aprior=use.aprior, use.bprior=use.bprior, use.gprior=use.gprior,
                           gradient=grad_item_drm,
    ...
    if(mod %in% c("1PLM", "2PLM", "3PLM")) {
      ...
      if(iter < 3) {
        tmp_est <- vector('list', 3)
        tmp_est[[1]] <- stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=quadpt, mod=mod, D=D,
        ...
        tmp_est[[2]] <- stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=quadpt, mod=mod, D=D,
                                      nstd=n.quad, fix.a=fix.a.1pl, fix.g=fix.g, a.val=a.val.1pl, g.val=g.val, n.1PLM=NULL,
                                      aprior=aprior, bprior=bprior, gprior=gprior,
                                      use.aprior=use.aprior, use.bprior=use.bprior, use.gprior=use.gprior,
                                      gradient=grad_item_drm,
                                      ...
                                      lower=lower, upper=upper)
        tmp_est[[3]] <- stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=quadpt, mod=mod, D=D,
                                      nstd=n.quad, fix.a=fix.a.1pl, fix.g=fix.g, a.val=a.val.1pl, g.val=g.val, n.1PLM=NULL,
                                      aprior=aprior, bprior=bprior, gprior=gprior,
                                      use.aprior=use.aprior, use.bprior=use.bprior, use.gprior=use.gprior,
                                      gradient=grad_item_drm,
      ...
      # estimate the item parameters
      est <- tryCatch({stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=quadpt, mod=mod, D=D,
                                     nstd=n.quad, fix.a=fix.a.1pl, fix.g=fix.g, a.val=a.val.1pl, g.val=g.val, n.1PLM=NULL,
                                     aprior=aprior, bprior=bprior, gprior=gprior,
                                     use.aprior=use.aprior, use.bprior=use.bprior, use.gprior=use.gprior,
                                     gradient=grad_item_drm,
      ...
      if(is.null(est) || est$convergence > 0L) {
        # if error or non-convergence, only use the gradient
        est <- stats::nlminb(startval, objective=loglike_drm, f_i=f_i, r_i=r_i, s_i=s_i, theta=quadpt, mod=mod, D=D,
                             nstd=n.quad, fix.a=fix.a.1pl, fix.g=fix.g, a.val=a.val.1pl, g.val=g.val, n.1PLM=NULL,
                             aprior=aprior, bprior=bprior, gprior=gprior,
                             use.aprior=use.aprior, use.bprior=use.bprior, use.gprior=use.gprior,
                             gradient=grad_item_drm,
      ...
      if(iter < 3) {
        tmp_est <- vector('list', 3)
        tmp_est[[1]] <- stats::nlminb(startval, objective=loglike_prm, r_i=r_i, theta=quadpt, pr.mod=mod, D=D,
                                      ...
                                      lower=lower, upper=upper)
        tmp_est[[2]] <- stats::nlminb(startval, objective=loglike_prm, r_i=r_i, theta=quadpt, pr.mod=mod, D=D,
                                      nstd=n.quad, fix.a=fix.a.gpcm, a.val=a.val.gpcm,
                                      aprior=aprior, bprior=bprior, use.aprior=use.aprior, use.bprior=use.bprior,
                                      gradient=grad_item_prm,
                                      control=list(eval.max=40-(iter * 10), iter.max=25-(iter*5), step.min=1, step.max=2, trace=0),
                                      lower=lower, upper=upper)
        tmp_est[[3]] <- stats::nlminb(startval, objective=loglike_prm, r_i=r_i, theta=quadpt, pr.mod=mod, D=D,
                                      nstd=n.quad, fix.a=fix.a.gpcm, a.val=a.val.gpcm,
                                      aprior=aprior, bprior=bprior, use.aprior=use.aprior, use.bprior=use.bprior,
                                      gradient=grad_item_prm,
      ...
      # estimate the item parameters
      est <- tryCatch({stats::nlminb(startval, objective=loglike_prm, r_i=r_i, theta=quadpt, pr.mod=mod, D=D,
                                     nstd=n.quad, fix.a=fix.a.gpcm, a.val=a.val.gpcm,
                                     aprior=aprior, bprior=bprior, use.aprior=use.aprior, use.bprior=use.bprior,
                                     gradient=grad_item_prm,
                                     hessian=hess_item_prm,
      ...
      if(is.null(est) || est$convergence > 0L) {
        # if error or non-convergence, only use the gradient
        est <- stats::nlminb(startval, objective=loglike_prm, r_i=r_i, theta=quadpt, pr.mod=mod, D=D,
                             nstd=n.quad, fix.a=fix.a.gpcm, a.val=a.val.gpcm,
                             aprior=aprior, bprior=bprior, use.aprior=use.aprior, use.bprior=use.bprior,
                             gradient=grad_item_prm,
                             # hessian=hess_item_prm,
