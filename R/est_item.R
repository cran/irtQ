...
est_item <- function(x=NULL, data, score, D=1, model=NULL, cats=NULL, item.id=NULL, fix.a.1pl=FALSE, fix.a.gpcm=FALSE,
  ...
  if(!is.null(loc_1p_const)) {
    ...
    est <- estimation1(f_i=f_i, r_i=r_i, s_i, theta=theta, mod="1PLM", D=D, nstd=nstd,
  ...
  if(length(loc_else) >= 1) {
    for(i in 1:length(loc_else)) {
      ...
      if(score.cat == 2) {
        ...
        # parameter estimation
        est <- estimation1(f_i=f_i, r_i=r_i, s_i=s_i, theta=theta, mod=mod, D=D,
      ...
      if(score.cat > 2) {
        ...
        # parameter estimation
        est <- estimation1(r_i=r_i, theta=theta, mod=mod, score.cat=score.cat, D=D,
