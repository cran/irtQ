...
info_xpd <- function(elm_item, freq.cat, post_dist, quadpt.vec, n.quadpt.vec, nstd,
  ...
  if(!is.null(loc_1p_const)) {
    ...
    grad <-
      unname(grad_llike(item_par=item_par, f_i=f_i, r_i=r_i, theta=quadpt.vec,
  ...
  if(length(loc_else) >= 1) {
    for(i in 1:length(loc_else)) {
      ...
      if(score.cat > 2) {
        ...
        # compute the gradient vectors
        grad <-
          unname(grad_llike(item_par=item_par, r_i=r_i, theta=quadpt.vec, n.theta=n.quadpt.vec,
...
info_prior <- function(elm_item, D=1, loc_1p_const, loc_else, n.1PLM,
  ...
  if(!is.null(loc_1p_const)) {
    ...
    hess <- hess_prior(item_par=item_par, mod="1PLM", D=D, fix.a.1pl=fix.a.1pl,
  ...
  if(length(loc_else) >= 1) {
    for(i in 1:length(loc_else)) {
      ...
      if(score.cat == 2) {
        ...
        # compute the gradient vectors
        hess <- hess_prior(item_par=item_par, mod=mod, D=D, fix.a.1pl=ifelse(mod == "1PLM", TRUE, FALSE),
      ...
      if(score.cat > 2) {
        ...
        # compute the gradient vectors
        hess <- hess_prior(item_par=item_par, mod=mod, D=D, fix.a.gpcm=ifelse(mod == "GPCM", fix.a.gpcm, FALSE),
