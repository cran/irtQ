...
print.catsib <- function(x, digits = max(2L, getOption("digits") - 5L), ...) {
  ...
  dif_stat_nopurify <-
    x$no_purify$dif_stat %>%
    dplyr::select("id", "n.ref", "n.foc", "n.total",
                  "beta", "se", "z.beta", "p") %>%
  ...
  if(purify) {
    ...
    dif_stat_purify <-
      x$with_purify$dif_stat %>%
      dplyr::select("id", "n.iter", "n.ref", "n.foc", "n.total",
                    "beta", "se", "z.beta", "p") %>%
...
print.rdif <- function(x, digits = max(2L, getOption("digits") - 5L), ...) {
  ...
  dif_stat_nopurify <-
    x$no_purify$dif_stat %>%
    dplyr::select("id", "n.ref", "n.foc", "rdifr",
                  "p.rdifr", "rdifs", "p.rdifs",
                  "rdifrs", "p.rdifrs") %>%
    dplyr::mutate_at(.vars = 4:9, round, digits = 3) %>%
    dplyr::mutate(" "=stats::symnum(.data$p.rdifr, c(0, 0.001, 0.01, 0.05, 0.1, 1),
                                    symbols = c("***", "**", "*", ".", "")),
  ...
  if(purify) {
    ...
    dif_stat_purify <-
      x$with_purify$dif_stat %>%
      dplyr::select("id", "n.iter", "n.ref", "n.foc",
