...
Estep_fipc <- function(elm_item1, elm_item2, idx.drm2, idx.prm2,
  ...
  if(is.null(idx.std)) {
    L_LL <- likelihood(elm_item2, idx.drm=idx.drm2, idx.prm=idx.prm2,
  ...
  # return results
  rst <- list(elm_item=elm_item1, post_dist=post_dist, freq.exp=freq.exp,
...
Estep <- function(elm_item, idx.drm=NULL, idx.prm=NULL,
  ...
  if(is.null(idx.std)) {
    L_LL <- likelihood(elm_item, idx.drm=idx.drm, idx.prm=idx.prm,
  ...
  # return results
  rst <- list(elm_item=elm_item, post_dist=post_dist, freq.exp=freq.exp,
              loglikehd=L_LL$LL, likehd=L_LL$L, idx.std=idx.std)
  rst
...
Mstep <- function(estep, id, cats, model, quadpt, n.quad, D=1, cols.item=NULL, loc_1p_const, loc_else, idx4est=NULL,
  ...
  if(!is.null(elm_item)) {
    ...
    if(!is.null(loc_1p_const)) {
      ...
      startval <-
        set_startval(pars=elm_item$pars, item=loc_1p_const,
      ...
      est <- estimation2(f_i=f_i, r_i=r_i, s_i=s_i, quadpt=quadpt, mod="1PLM", D=D, n.quad=n.quad,
    ...
    if(length(loc_else) >= 1) {
      for(i in 1:length(loc_else)) {
        ...
        if(score.cat == 2) {
          ...
          # set the starting values
          startval <-
            set_startval(pars=elm_item$pars, item=loc_else[i],
          ...
          # item parameter estimation
          est <- estimation2(f_i=f_i, r_i=r_i, s_i=s_i, quadpt=quadpt, mod=mod, D=D,
        ...
        if(score.cat > 2) {
          ...
          # item parameter estimation
          est <- estimation2(r_i=r_i, quadpt=quadpt, mod=mod, score.cat=score.cat, D=D,
  ...
  if(EmpHist) {
    ...
    if(is.null(idx.std)) {
      ...
      prior_freq <- prior_freq2 <-
        purrr::map(.x = post_dist, ~{unname(Rfast::colsums(.x))})
  ...
  rst <- list(elm_item=elm_item, convergence=convergence, noconv_items=noconv_items,
