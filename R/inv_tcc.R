...
inv_tcc <- function(x, data, D=1, intpol=TRUE, range.tcc=c(-7, 7), tol=1e-4, max.it=500) {
  ...
  th4obs <- purrr::map_dbl(.x=obs.score[obs2theta.lg],
                           .f=function(x) {
                             loc.node <- which(diff(sign(x - tcc.vals)) != 0)
                             bd <- theta.nodes[c(loc.node, loc.node + 1)]
                             bisection(.fun=f.o2t, tau=x, lb=bd[1], ub=bd[2], tol=tol,
                                       max.it=max.it)$root
                           })
  ...
  if(intpol) {
    ...
    if(range.tcc[1] > th4obs[1]) {
      ...
      warning(memo, call.=FALSE)
    } else if(range.tcc[2] < tail(th4obs, 1)) {
      memo <- paste0("An upper bound of theta must be greater than ", round(tail(th4obs, 1), 3), "\n",
